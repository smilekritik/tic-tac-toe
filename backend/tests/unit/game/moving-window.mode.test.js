const movingWindowMode = require('../../../src/modules/game/engine/moving-window.mode');
...
describe('moving-window mode', () => {
  ...
  it('never reports draw', () => {
    expect(movingWindowMode.checkDraw()).toBe(false);
  });

  it('evaluates wins on the current board instead of historical moves', () => {
    let state = movingWindowMode.initGame();
    state = movingWindowMode.applyMove(state, 0, 'X');
    state = movingWindowMode.applyMove(state, 3, 'O');
